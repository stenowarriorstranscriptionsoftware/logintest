...
function loadTestDropdown() {
  ...
  const savedTests = JSON.parse(localStorage.getItem('customTests') || '{}');
  Object.keys(savedTests).forEach(title => {
...
saveBtn.addEventListener('click', () => {
  ...
  const saved = JSON.parse(localStorage.getItem('customTests') || '{}');
  saved[title] = text;
  localStorage.setItem('customTests', JSON.stringify(saved));
  loadTestDropdown();
  customTitle.value = '';
...
loadBtn.addEventListener('click', () => {
  const selectedTitle = testSelect.value;
  const saved = JSON.parse(localStorage.getItem('customTests') || '{}');
  if (selectedTitle && saved[selectedTitle]) {
    document.getElementById('originalText').value = saved[selectedTitle];
    document.getElementById('originalTextGroup').classList.remove('hidden');
    alert(`Loaded test: ${selectedTitle}`);
  }
});

clearBtn.addEventListener('click', () => {
  if (confirm('Are you sure you want to delete all saved tests?')) {
    localStorage.removeItem('customTests');
    loadTestDropdown();
  }
});
